...
int uprobe__libc_open(struct pt_regs *ctx)
{
...
SEC("uprobe/libc.so.6:openat")
int uprobe__libc_openat(struct pt_regs *ctx)
{
	u64 id = bpf_get_current_pid_tgid();
...
static __always_inline int trace_exit(struct pt_regs *ctx)
{
...
SEC("uretprobe/libc.so.6:open")
int uretprobe__libc_open(struct pt_regs *ctx)
{
	return trace_exit(ctx);
}

SEC("uretprobe/libc.so.6:openat")
int uretprobe__libc_openat(struct pt_regs *ctx)
{
	return trace_exit(ctx);
}

SEC("uprobe/libc.so.6:close")
int uprobe__libc_close(struct pt_regs *ctx)
{
	int fd = (int)ctx->di;
...
SEC("uprobe/libc.so.6:getdents64")
int uprobe__libc_getdents64(struct pt_regs *ctx)
{
	int fd = (int)ctx->di;
	...
	args.fd = fd;
	args.dirp = (void *)ctx->si;
	args.count = (unsigned int)ctx->dx;
...
int uretprobe__libc_getdents64(struct pt_regs *ctx)
{
	int ret = ctx->ret;
	if (ret < 0)
	{
...
SEC("uprobe/libc.so.6:newfstatat")
int uprobe__libc_newfstatat(struct pt_regs *ctx)
{
	int fd = (int)ctx->di;
	struct dir_fd_data *dir_data = get_dir_fd_data(fd);
	if (dir_data == NULL)
	...
	const char *path = (const char *)ctx->si;
...
SEC("uprobe/libc.so.6:statfs")
int uprobe__libc_statfs(struct pt_regs *ctx)
{
...
int uprobe__libc_fcntl(struct pt_regs *ctx)
{
	int fd = (int)ctx->di;
	int cmd = (int)ctx->si;
	if (cmd != F_DUPFD)
...
SEC("uretprobe/libc.so.6:fcntl")
int uretprobe__libc_fcntl(struct pt_regs *ctx)
{
	int ret = ctx->rax;
	if (ret < 0)
	{
